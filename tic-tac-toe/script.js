...
let origBoard; //keep track of X and O. what's in each square board 
let huPlayer ='O'; //human player 
let aiPlayer = 'X'; //ai player
const winCombos =[
  [0, 1, 2],
  [3, 4, 5],
  [6, 7, 8],
...
function selectSym(sym){
  huPlayer = sym;
  aiPlayer = sym==='O' ? 'X' :'O'; //assigning symbol to ai player using turnary operator
...
function checkTie() {
  if (emptySquares().length === 0){
    for (cell of cells) {
      cell.style.backgroundColor = "#639a67";
